...
export default makeScene2D(function* (view) {
  ...
  const dupeCheckTxt = createRef<Txt>();
  const timeTextSignal = createSignal(0);
  ...
  view.add(
    ...
    const currentX = globalCheck().position.x();
    const currentY = globalCheck().position.y();
  yield* chain(
    all(timeText().opacity(0, 1), timeText().position([timeText().position.x(), 0], 1)),
    globalCheck().opacity(1, .5),
    waitFor(.4),
    all(
      globalCheck().scale(2.5, .5, easeOutCubic),
      globalCheck().position([currentX, currentY - 9], .3),
      globalCheck().opacity(.3, .5)),
    all(
      globalCheck().scale(2, .5, easeOutBounce),
      globalCheck().position([currentX, currentY], .5),
      globalCheck().opacity(1, .5)));
